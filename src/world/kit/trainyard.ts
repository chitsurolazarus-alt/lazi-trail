...
function canopy(f: Frame, a0: number, a1: number): void {
  ...
  f.box('metal', a0, a1, top - 0.25, top, -3.2, -3.1, 0x33373d, [
    'pz',
    'nz',
    f.side === -1 ? 'px' : 'nx',
  ]);
...
function warehouse(f: Frame, a0: number, a1: number): void {
  ...
  const tint =
    wall === 'corrugated'
      ? pickOne(rng, [0x9aa1a8, 0x6f8a9a, 0xb3543a, 0x8a8f96])
      : shade(0xcfcac0, rng, 0.08);
  ...
  if (rng() < 0.6)
    f.atlasFace(
      ...
      5.9,
      0.16,
      f.signs.graffitiRect(Math.floor(rng() * f.signs.graffitiCount)),
    );
}
...
function gantry(f: Frame, a: number): void {
  ...
  if (f.side === 1) {
    ...
    b.box(0, h - 0.4, f.z(a), Math.abs(xL - xR), 0.28, 0.28, 0x565c63, {
      faces: ['py', 'ny', 'pz', 'nz'],
      tile: 2.5,
    });
...
function wires(f: Frame): void {
  ...
  const m = f.k.of('matte');
  for (const x of [-1.2, 1.2]) {
    m.box(x, 6.0, -CHUNK_LENGTH / 2, 0.04, 0.04, CHUNK_LENGTH, 0x1e2024, {
      faces: ['py', 'px', 'nx'],
    });
    m.box(x, 5.4, -CHUNK_LENGTH / 2, 0.035, 0.035, CHUNK_LENGTH, 0x1e2024, {
      faces: ['py', 'px', 'nx'],
    });
  }
}
