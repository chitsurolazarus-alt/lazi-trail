...
function floodlightMast(f: Frame, a: number, height: number): void {
  const d = -2.2;
  f.box('matte', a - 0.2, a + 0.2, 0, height, d - 0.2, d + 0.2, 0x8a9199, ['py', 'pz', 'nz', 'px', 'nx']);
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      lamp.box(f.x(d - 0.3), hh, f.z(aa), 0.25, 0.4, 0.6, 0xfff2cf, { faces: [f.side === -1 ? 'px' : 'nx', 'py', 'pz', 'nz'] });
    }
  }
...
function flagLine(f: Frame, a0: number, a1: number): void {
  for (let a = a0; a <= a1; a += 5.5) {
    const d = -3.1;
    f.box('matte', a - 0.05, a + 0.05, 0, 5.6, d - 0.05, d + 0.05, 0xc9ced4, ['py', 'pz', 'nz', 'px', 'nx']);
    const c = pickOne(f.rng, FLAG_COLORS);
    const x = f.x(d);
    const lo = f.side === -1 ? a + 0.05 : a + 1.75;
    const hi = f.side === -1 ? a + 1.75 : a + 0.05;
    f.k.of('cloth').quad([x, 3.5, f.z(lo)], [x, 3.5, f.z(hi)], [x, 5.5, f.z(hi)], [x, 5.5, f.z(lo)], c, 1, 1);
  }
}
...
function concourse(f: Frame, a0: number, a1: number): void {
  ...
  if (rng() < 0.7) {
    ...
    f.atlasFace(a0 + 1.15, a1 - 1.15, H - 4.25, H - 0.75, -0.52, f.signs.billboardRect(Math.floor(rng() * 6)));
  }
}
