...
import { CHUNK_LENGTH, Frame, MUTED, PAINTS, parkedCar, randomCarColor, shade } from './common';
...
function officeWindow(
  ...
  f.box('concrete', a0 - 0.14, a1 + 0.14, h0 - 0.16, h0 - 0.06, -0.24, 0, 0xcfcac0, [
    'py',
    'pz',
    'nz',
    f.side === -1 ? 'px' : 'nx',
...
function balcony(f: Frame, aC: number, floorBase: number, w: number): void {
  ...
  // railing: rail + posts
  f.box('metal', a0, a1, floorBase + 0.98, floorBase + 1.04, -1.15, -1.09, 0x222428, [
    'py',
    'pz',
    'nz',
    f.side === -1 ? 'px' : 'nx',
  ...
  for (let i = 0; i <= n; i++) {
    const a = a0 + (w * i) / n;
    f.box('matte', a - 0.015, a + 0.015, floorBase, floorBase + 1.0, -1.15, -1.09, 0x222428, [
      f.side === -1 ? 'px' : 'nx',
    ]);
  }
  if (f.rng() < 0.4)
    f.box('matte', aC - 0.35, aC + 0.35, floorBase + 0.05, floorBase + 0.75, -0.9, -0.5, 0xdadbd8); // AC unit
}
...
function tower(f: Frame, a0: number, a1: number, floors: number): void {
  ...
  const tint =
    mat === 'plaster'
      ? pickOne(rng, [...MUTED, ...PAINTS.slice(0, 4)])
      : mat === 'brick'
        ? shade(0xf0d2c0, rng, 0.1)
        : shade(0xd8d6d0, rng, 0.08);
  ...
  for (let fl = 0; fl < floors; fl++) {
    ...
    for (let c = 0; c < cols; c++) {
      ...
      if (balconyFloors.has(fl) && c % 2 === 0)
        balcony(f, aC, base + 0.2, Math.min(1.9, colW * 0.85));
    }
    f.box('concrete', a0, a1, base + FLOOR_H - 0.1, base + FLOOR_H + 0.06, -0.08, 0.05, 0xd6d1c6, [
      'pz',
      'nz',
      f.side === -1 ? 'px' : 'nx',
    ]);
  ...
  if (rng() < 0.5)
    f.box('matte', a0 + w * 0.5 - 0.03, a0 + w * 0.5 + 0.03, H, H + 4, 6, 6.06, 0x333333, [
      'pz',
      'nz',
      'px',
      'nx',
